...
def create_overview_cards(df):
    ...
    if df.empty:
        return {"total_matches": 0, "total_kills": 0, "avg_kd_ratio": 0}
...
def create_mode_wise_analysis(df):
    ...
    mode_stats["matches_per_player"] = (
        mode_stats["match_id"] / mode_stats["player_name"]
    )
    ...
    fig.add_trace(
        go.Bar(
            x=mode_stats["game_mode"],
            y=mode_stats["matches_per_player"],
            name="Matches/Player",
            marker_color="#1f77b4",
        ),
        row=1,
...
def create_map_wise_analysis(df):
    ...
    heatmap_data = map_stats[
        ["map_name", "kd_ratio", "kills_per_minute", "score_per_minute"]
    ].set_index("map_name")
...
def create_weapon_map_analysis(df):
    ...
    weapon_map_stats["kills_per_minute"] = (
        weapon_map_stats["kills"] / weapon_map_stats["match_length"]
    )
    weapon_map_stats["score_per_minute"] = (
        weapon_map_stats["score"] / weapon_map_stats["match_length"]
    )

    # Create pivot table for heatmap
    pivot_kd = weapon_map_stats.pivot(
        index="map_name", columns="weapon", values="kd_ratio"
    ).fillna(0)
    pivot_kills_per_min = weapon_map_stats.pivot(
        index="map_name", columns="weapon", values="kills_per_minute"
    ).fillna(0)
    ...
    fig.update_layout(
        title="Weapon-Map Effectiveness Analysis (Per-Minute Metrics)", height=500
    )
...
def create_weapon_usage_chart(df):
    ...
    fig = go.Figure(
        data=[
            go.Bar(x=weapons, y=usage_counts, name="Usage Count", yaxis="y"),
            go.Scatter(
                x=weapons,
                y=kd_ratios,
                name="K/D Ratio",
...
def create_map_performance_chart(df):
    ...
    fig = go.Figure(
        data=[
            go.Bar(x=maps, y=matches_played, name="Matches Played", yaxis="y"),
            go.Scatter(
                x=maps,
                y=avg_kills,
    ...
    fig.update_layout(
        ...
        yaxis2=dict(title="Avg Kills per Match", overlaying="y", side="right"),
        height=400,
        showlegend=True,
    )
...
def create_player_comparison_radar(df, players):
    ...
    for player in players:
        ...
        if player_stats:
            ...
            fig.add_trace(
                go.Scatterpolar(r=values, theta=categories, fill="toself", name=player)
            )
    ...
    fig.update_layout(
        polar=dict(
            radialaxis=dict(
                ...
                range=[
                    0,
                    max([get_player_stats(df, p).get("kd_ratio", 0) for p in players]),
                ],
            )
        ),
        showlegend=True,
...
def create_match_timeline(df):
    ...
    match_summaries["kills_per_minute"] = (
        match_summaries["kills"] / match_summaries["match_length"]
    )
    match_summaries["deaths_per_minute"] = (
        match_summaries["deaths"] / match_summaries["match_length"]
    )
    match_summaries["score_per_minute"] = (
        match_summaries["score"] / match_summaries["match_length"]
    )
    ...
    match_summaries["datetime"] = pd.to_datetime(
        match_summaries["datetime"], errors="coerce"
    )
    if hasattr(match_summaries["datetime"].dt, "tz_localize"):
        if match_summaries["datetime"].dt.tz is not None or any(
            getattr(x, "tzinfo", None) is not None
            for x in match_summaries["datetime"]
            if pd.notnull(x)
        ):
            match_summaries["datetime"] = match_summaries["datetime"].dt.tz_localize(
                None
            )
    ...
    if len(match_summaries) > 3:
        ...
        # K/D ratio trend
        kd_trend = (
            match_summaries["kd_ratio"].rolling(window=window_size, center=True).mean()
        )
        fig.add_trace(
            go.Scatter(
                x=match_summaries["datetime"],
...
def create_detailed_match_timeline(df):
    ...
    match_summaries["efficiency"] = match_summaries["score"] / (
        match_summaries["kills"] + match_summaries["deaths"]
    ).replace(0, 1)
    ...
    fig.update_layout(
        ...
        yaxis2=dict(title="K/D Ratio", overlaying="y", side="right"),
        yaxis3=dict(
            title="Score per Minute", overlaying="y", side="right", position=0.95
        ),
...
def create_ping_impact_chart(df):
    ...
    fig.update_layout(
        ...
        yaxis_title="K/D Ratio",
        yaxis2=dict(title="Avg Score/Min", overlaying="y", side="right"),
        height=400,
        showlegend=True,
    )
...
def create_player_evolution_chart(df, player_name):
    ...
    fig.update_layout(
        ...
        xaxis_title="Date",
        yaxis_title="K/D Ratio",
        yaxis2=dict(title="Kills per Minute", overlaying="y", side="right"),
        height=500,
        showlegend=True,
    )
        ...
        if stats["players"]:
            fig.add_trace(
                go.Scatter(
                    ...
                    text=stats["players"],
                    marker=dict(
                        size=15, color=colors[cluster_id % len(colors)], opacity=0.7
                    ),
...
def create_streak_analysis_chart(df, player_name):
    ...
    values = [
        ...
        streak_data["max_win_streak"],
        streak_data["max_loss_streak"],
    ]
    colors = [
        "#2ca02c" if streak_data["current_streak"] > 0 else "#d62728",
        "#2ca02c",
        "#d62728",
    ]
    ...
    for player1 in players:
        ...
        for player2 in players:
            if player1 == player2:
                ...
                text_row.append(
                    f"{chemistry_matrix[player1][player2]['win_rate']:.1f}%"
                )
            else:
...
def create_role_analysis_chart(df):
    ...
    for player, analysis in role_analysis.items():
        ...
        values = [
            strengths["killing_power"],
            strengths["support_value"],
            strengths["survival_rate"],
            strengths["winning_ability"],
    ...
    fig = go.Figure(
        data=[
            go.Scatter(
                ...
                marker=dict(
                    size=[
                        count * 5 for count in match_counts
                    ],  # Size based on match count
                    color=win_rates,
                    colorscale="RdYlGn",
                    showscale=True,
        ...
        if players:
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    mode="markers+text",
                    marker=dict(
                        size=20, color=tier_colors[tier_name], symbol="diamond"
                    ),
                    text=[p["player_name"] for p in players],
                    textposition="middle center",
                    name=tier_name,
                    hovertemplate="<b>%{text}</b><br>Ranking Score: %{customdata}<br>Tier: "
                    + tier_name
                    + "<extra></extra>",
        ...
        data=[
            go.Bar(
                ...
                marker_color=[
                    (
                        "#4CAF50"
                        if count == total_achievements
                        else "#FFC107" if count > total_achievements / 2 else "#F44336"
                    )
                    for count in unlocked_counts
                ],
                text=unlocked_counts,
                textposition="auto",
                hovertemplate="<b>%{x}</b><br>Achievements: %{y}/"
                + str(total_achievements)
                + "<extra></extra>",
            )
        ]
    )
...
def create_gaming_session_analysis_chart(df):
    ...
    if session_analysis:
        ...
        fig.add_trace(
            go.Bar(
                x=[f"Session {sid}" for sid in session_ids],
                y=durations,
    ...
    fig.update_layout(title="🎮 Gaming Session Analysis", height=600, showlegend=True)
...
def create_player_comparison_chart(df, player1, player2):
    ...
    fig.add_trace(
        go.Bar(
            ...
            marker_color="#1f77b4",
            text=[
                f"{v:.2f}" if isinstance(v, float) else str(v) for v in player1_values
            ],
            textposition="auto",
        )
    )
    ...
    fig.add_trace(
        go.Bar(
            ...
            marker_color="#ff7f0e",
            text=[
                f"{v:.2f}" if isinstance(v, float) else str(v) for v in player2_values
            ],
            textposition="auto",
        )
    )
...
def create_scenario_simulation_chart(df, team_composition, opponent_composition=None):
    ...
    team_performance = simulate_team_scenario(
        df, team_composition, opponent_composition
    )
    ...
    normalized_values = [
        min(v / max_val, 1.0) for v, max_val in zip(values, max_values)
    ]
    ...
    if opponent_composition and "head_to_head_win_rate" in team_performance:
        ...
        if opponent_performance:
            ...
            opponent_normalized = [
                min(v / max_val, 1.0) for v, max_val in zip(opponent_values, max_values)
            ]
...
def create_optimal_team_chart(df, available_players, team_size=3):
    ...
    sorted_teams = sorted(
        optimal_data["all_teams"].items(), key=lambda x: x[1]["score"], reverse=True
    )[:10]
    ...
    fig.add_trace(
        go.Scatter(
            ...
            marker=dict(
                size=[
                    score * 2 for score in team_scores
                ],  # Size based on overall score
                color=team_scores,
                colorscale="RdYlGn",
                showscale=True,
