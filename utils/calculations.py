...
def get_player_evolution_timeline(df, player_name):
    ...
    if hasattr(match_stats["datetime"].dt, "tz_localize"):
        if match_stats["datetime"].dt.tz is not None or any(
            getattr(x, "tzinfo", None) is not None
            for x in match_stats["datetime"]
            if pd.notnull(x)
        ):
    ...
    match_stats["deaths_per_minute"] = (
        match_stats["deaths"] / match_stats["match_length"]
    )
    match_stats["assists_per_minute"] = (
        match_stats["assists"] / match_stats["match_length"]
    )
    ...
    match_stats["kd_trend"] = (
        match_stats["kd_ratio"].rolling(window=3, min_periods=1).mean()
    )
    match_stats["kills_trend"] = (
        match_stats["kills_per_minute"].rolling(window=3, min_periods=1).mean()
    )
    match_stats["score_trend"] = (
        match_stats["score_per_minute"].rolling(window=3, min_periods=1).mean()
    )
...
def get_performance_clusters(df, n_clusters=3):
    ...
    for cluster_id in range(n_clusters):
        ...
        cluster_stats[cluster_id] = {
            ...
            "avg_assists_per_min": round(
                cluster_players["assists_per_minute"].mean(), 2
            ),
...
def get_player_streaks(df, player_name):
    ...
    for match_id in player_matches:
        ...
        if game_mode in ["Team", "Team Confirm"]:
            player_team = match_data[match_data["player_name"] == player_name][
                "team"
            ].iloc[0]

            if game_mode == "Team":
                # Regular team mode - winner by team score
                team_score = match_data[match_data["team"] == player_team][
                    "score"
                ].sum()
            ...
            if (
                not player_row.empty
                and "tags" in player_row
                and pd.notna(player_row["tags"].iloc[0])
            ):
            ...
            # FFA match
            player_score = match_data[match_data["player_name"] == player_name][
                "score"
            ].iloc[0]
    ...
    for result in match_results:
        try:
            result["datetime"] = pd.to_datetime(
                result["datetime"], format="mixed", errors="coerce"
            )
        except Exception:
            try:
                result["datetime"] = pd.to_datetime(
                    result["datetime"], format="ISO8601", errors="coerce"
                )
            except Exception:
    ...
    for result in match_results:
        if (
            hasattr(result["datetime"], "tz_localize")
            and result["datetime"].tz is not None
        ):
    ...
    if match_results:
        current_streak = (
            current_win_streak if match_results[-1]["won"] else -current_loss_streak
        )

    # Calculate win rate in recent matches
    recent_matches = match_results[-10:] if len(match_results) >= 10 else match_results
    recent_win_rate = (
        sum(1 for r in recent_matches if r["won"]) / len(recent_matches) * 100
    )
...
def calculate_player_wins(df, player_name):
    ...
    for match_id in player_matches:
        ...
        if game_mode in ["Team", "Team Confirm"]:
            ...
            # Confirm mode - winner by individual tags
            player_tags = match_data[match_data["player_name"] == player_name][
                "tags"
            ].iloc[0]
            max_tags = match_data["tags"].max()
...
def get_player_stats(df, player_name=None):
    ...
    stats = {
        ...
        "total_assists": (
            ...
            if "assists" in player_df.columns and not player_df["assists"].isna().all()
            else 0
        ),
        "total_score": int(player_df["score"].sum()),
        "total_coins": (
            int(player_df["coins"].sum()) if "coins" in player_df.columns else 0
        ),
        "total_tags": (
            int(player_df["tags"].sum())
            if "tags" in player_df.columns and not player_df["tags"].isna().all()
            else 0
        ),
        ...
        "losses": losses,
        "win_rate": win_rate,
        "avg_kills_per_match": round(
            player_df.groupby("match_id")["kills"].sum().mean(), 1
        ),
        "avg_deaths_per_match": round(
            player_df.groupby("match_id")["deaths"].sum().mean(), 1
        ),
        "avg_assists_per_match": (
            round(player_df.groupby("match_id")["assists"].sum().mean(), 1)
            if "assists" in player_df.columns and not player_df["assists"].isna().all()
            else 0
        ),
        "avg_score_per_match": round(
            player_df.groupby("match_id")["score"].sum().mean(), 1
        ),
        "avg_tags_per_match": (
            round(player_df.groupby("match_id")["tags"].sum().mean(), 1)
            if "tags" in player_df.columns and not player_df["tags"].isna().all()
            else 0
        ),
        "kills_per_minute": (
            round(player_df["kills"].sum() / total_minutes, 2)
            if total_minutes > 0
            else 0
        ),
        "deaths_per_minute": (
            round(player_df["deaths"].sum() / total_minutes, 2)
            if total_minutes > 0
            else 0
        ),
        "assists_per_minute": (
            round(player_df["assists"].sum() / total_minutes, 2)
            if "assists" in player_df.columns
            and not player_df["assists"].isna().all()
            and total_minutes > 0
            else 0
        ),
        "score_per_minute": (
            round(player_df["score"].sum() / total_minutes, 2)
            if total_minutes > 0
            else 0
        ),
        "tags_per_minute": (
            round(player_df["tags"].sum() / total_minutes, 2)
            if "tags" in player_df.columns
            and not player_df["tags"].isna().all()
            and total_minutes > 0
            else 0
        ),
        "kd_ratio": calculate_kd_ratio(
            player_df["kills"].sum(), player_df["deaths"].sum()
        ),
        ...
        "best_match_assists": (
            int(player_df.groupby("match_id")["assists"].sum().max())
            if "assists" in player_df.columns and not player_df["assists"].isna().all()
            else 0
        ),
        "best_match_tags": (
            int(player_df.groupby("match_id")["tags"].sum().max())
            if "tags" in player_df.columns and not player_df["tags"].isna().all()
            else 0
        ),
        "favorite_weapon": (
            player_df["weapon"].mode().iloc[0]
            if not player_df["weapon"].mode().empty
            else "Unknown"
        ),
        "avg_ping": (
            round(player_df["ping"].mean(), 1)
            if "ping" in player_df.columns and not player_df["ping"].isna().all()
            else None
        ),
    }
...
def get_team_stats(df):
    ...
    for team in team_df["team"].unique():
        ...
        for match_id in matches:
            ...
            other_teams = team_df[
                (team_df["match_id"] == match_id) & (team_df["team"] != team)
            ]
        ...
        team_stats[team] = {
            ...
            "losses": team_losses,
            "win_rate": (
                round((team_wins / (team_wins + team_losses)) * 100, 1)
            ...
            "avg_score_per_match": round(
                team_data.groupby("match_id")["score"].sum().mean(), 1
            ),
        }
...
def get_weapon_stats(df):
    ...
    for weapon in df["weapon"].unique():
        ...
        weapon_stats[weapon] = {
            ...
            "kd_ratio": calculate_kd_ratio(
                weapon_data["kills"].sum(), weapon_data["deaths"].sum()
            ),
...
def get_map_stats(df):
    ...
    for map_name in df["map_name"].unique():
        ...
        map_stats[map_name] = {
            ...
            "avg_kills_per_match": round(
                map_data.groupby("match_id")["kills"].sum().mean(), 1
            ),
            "avg_score_per_match": round(
                map_data.groupby("match_id")["score"].sum().mean(), 1
            ),
        }
...
def get_recent_activity(df, days=7):
    ...
    try:
        df_copy["datetime"] = pd.to_datetime(
            df_copy["datetime"], format="mixed", errors="coerce", utc=True
        )
    except Exception:
        try:
            df_copy["datetime"] = pd.to_datetime(
                df_copy["datetime"], format="ISO8601", errors="coerce", utc=True
            )
        except Exception:
            df_copy["datetime"] = pd.to_datetime(
                df_copy["datetime"], errors="coerce", utc=True
            )
...
def get_match_summary(df, match_id):
    ...
    summary = {
        ...
        "winner": winner,
        "top_killer": (
            match_data.loc[match_data["kills"].idxmax(), "player_name"]
            if match_data["kills"].max() > 0
            else "None"
        ),
        "players": match_data[
            ["player_name", "kills", "deaths", "assists", "score", "weapon"]
        ].to_dict("records"),
    }
    ...
    for player1 in team_players:
        ...
        for player2 in team_players:
            if player1 != player2:
                ...
                # Find common matches
                common_matches = set(player1_matches["match_id"]).intersection(
                    set(player2_matches["match_id"])
                )
                ...
                if common_matches:
                    ...
                    for match_id in common_matches:
                        ...
                        player1_team = match_data[match_data["player_name"] == player1][
                            "team"
                        ].iloc[0]
                        player2_team = match_data[match_data["player_name"] == player2][
                            "team"
                        ].iloc[0]

                        if player1_team == player2_team:
                            # They're on the same team
                            team_score = match_data[match_data["team"] == player1_team][
                                "score"
                            ].sum()
                            other_teams = match_data[match_data["team"] != player1_team]

                            if not other_teams.empty:
                                other_team_scores = other_teams.groupby("team")[
                                    "score"
                                ].sum()
                                max_other_score = other_team_scores.max()
                                won = team_score > max_other_score
    ...
    for player in players:
        ...
        if player_stats:
            ...
            if kd_ratio > 1.5 and kills_per_min > 0.8:
                primary_role = "Killer"
                role_description = (
                    "High K/D ratio and kill rate - primary damage dealer"
                )
            elif assists_per_min > 0.3 and deaths_per_min < 0.5:
                primary_role = "Support"
                role_description = (
                    "High assist rate with low deaths - team support player"
                )
            ...
            role_strengths = {
                ...
                "consistency": min(
                    player_stats["total_matches"] / 20.0, 1.0
...
def get_team_formation_performance(df):
    ...
    for match_id in team_matches["match_id"].unique():
        ...
        for team in teams:
            team_players = match_data[match_data["team"] == team][
                "player_name"
            ].tolist()
            ...
            if len(formation_key) >= 2:  # Only consider formations with 2+ players
                ...
                formation_stats[formation_key]["total_kills"] += match_data[
                    match_data["team"] == team
                ]["kills"].sum()
                formation_stats[formation_key]["total_score"] += match_data[
                    match_data["team"] == team
                ]["score"].sum()
    ...
    for formation_key, stats in formation_stats.items():
        if stats["matches"] > 0:
            ...
            stats["avg_kills_per_match"] = round(
                stats["total_kills"] / stats["matches"], 1
            )
            stats["avg_score_per_match"] = round(
                stats["total_score"] / stats["matches"], 1
            )
    ...
    sorted_formations = dict(
        sorted(sorted_formations.items(), key=lambda x: x[1]["win_rate"], reverse=True)
    )
...
def get_achievement_badges(df):
    ...
    achievement_criteria = {
        "Sharpshooter": {
            "metric": "kd_ratio",
            "threshold": 2.0,
            "description": "K/D ratio above 2.0",
        },
        "Kill Master": {
            "metric": "total_kills",
            "threshold": 100,
        ...
        "Survivor": {
            "metric": "deaths_per_minute",
            "threshold": 0.3,
            "description": "Less than 0.3 deaths per minute",
            "reverse": True,
        },
        "Support Hero": {
            "metric": "assists_per_minute",
            "threshold": 0.5,
            "description": "0.5+ assists per minute",
        },
        "Winner": {
            "metric": "win_rate",
            "threshold": 70,
        ...
        "Speed Demon": {
            "metric": "kills_per_minute",
            "threshold": 1.0,
            "description": "1.0+ kills per minute",
        },
        "Consistent": {
            "metric": "total_matches",
            "threshold": 10,
        ...
        "Champion": {
            "metric": "ranking_score",
            "threshold": 90,
            "description": "Champion tier ranking",
        },
    }
...
def get_gaming_session_analysis(df):
    ...
    for i in range(len(daily_stats)):
        if i == 0:
            ...
            days_diff = (
                daily_stats.iloc[i]["date"] - daily_stats.iloc[i - 1]["date"]
            ).days
    ...
    for session_id in daily_stats["session_id"].unique():
        ...
        session_analysis[session_id] = {
            ...
            "duration_days": (
                session_data["date"].max() - session_data["date"].min()
            ).days
            + 1,
            ...
            "peak_performance_day": session_data.loc[
                session_data["kd_ratio"].idxmax(), "date"
            ],
    ...
    hourly_performance = (
        df.groupby("hour")
        .agg({"kills": "mean", "deaths": "mean", "score": "mean", "match_id": "count"})
        .reset_index()
    )
    ...
    return {
        ...
        "avg_session_duration": (
            sum(s["duration_days"] for s in session_analysis.values())
...
def get_player_comparison_data(df, player1, player2):
    ...
    comparison_data = {
        "player1": {"name": player1, "stats": player1_stats},
        "player2": {"name": player2, "stats": player2_stats},
        "comparison": {
            "kd_ratio_diff": player1_stats["kd_ratio"] - player2_stats["kd_ratio"],
            "win_rate_diff": player1_stats["win_rate"] - player2_stats["win_rate"],
            "kills_per_min_diff": player1_stats["kills_per_minute"]
            - player2_stats["kills_per_minute"],
            "assists_per_min_diff": player1_stats["assists_per_minute"]
            - player2_stats["assists_per_minute"],
            "total_matches_diff": player1_stats["total_matches"]
            - player2_stats["total_matches"],
            "total_kills_diff": player1_stats["total_kills"]
            - player2_stats["total_kills"],
        },
    }
    ...
    comparison_data["winners"] = {
        ...
        "win_rate": (
            player1
            if player1_stats["win_rate"] > player2_stats["win_rate"]
            else player2
        ),
        "kills_per_min": (
            player1
            if player1_stats["kills_per_minute"] > player2_stats["kills_per_minute"]
            else player2
        ),
        "assists_per_min": (
            player1
            if player1_stats["assists_per_minute"] > player2_stats["assists_per_minute"]
            else player2
        ),
        "total_matches": (
            player1
            if player1_stats["total_matches"] > player2_stats["total_matches"]
            else player2
        ),
        "total_kills": (
            player1
            if player1_stats["total_kills"] > player2_stats["total_kills"]
            else player2
        ),
    }
...
def simulate_team_scenario(df, team_composition, opponent_composition=None):
    ...
    team_performance = {
        ...
        "avg_kd_ratio": sum(stats["kd_ratio"] for stats in team_stats.values())
        / len(team_stats),
        "total_win_rate": sum(stats["win_rate"] for stats in team_stats.values())
        / len(team_stats),
        "total_kills_per_min": sum(
            stats["kills_per_minute"] for stats in team_stats.values()
        ),
        "total_assists_per_min": sum(
            stats["assists_per_minute"] for stats in team_stats.values()
        ),
    ...
    base_win_prob = min(
        team_performance["avg_kd_ratio"] * 25
        + team_performance["total_win_rate"] * 0.5,
        95,
    )
    ...
    if opponent_composition:
        ...
        if opponent_stats:
            opponent_performance = {
                "avg_kd_ratio": sum(
                    stats["kd_ratio"] for stats in opponent_stats.values()
                )
                / len(opponent_stats),
                "total_win_rate": sum(
                    stats["win_rate"] for stats in opponent_stats.values()
                )
                / len(opponent_stats),
            }
            ...
            opponent_strength = min(
                opponent_performance["avg_kd_ratio"] * 25
                + opponent_performance["total_win_rate"] * 0.5,
                95,
            )
            ...
            team_performance["head_to_head_win_rate"] = max(
                min(head_to_head_win_rate, 95), 5
            )
...
def get_optimal_team_composition(df, available_players, team_size=3):
    ...
    for team in possible_teams:
        ...
        if team_performance:
            ...
            team_analysis[team] = {"performance": team_performance, "score": team_score}
