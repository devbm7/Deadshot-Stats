...
def test_validate_match_data():
    match_data = [
        {'player_name': 'Alice', 'kills': 5, 'deaths': 2, 'score': 100, 'weapon': 'AR', 'match_length': 10},
        {'player_name': 'Bob', 'kills': 3, 'deaths': 4, 'score': 80, 'weapon': 'SMG', 'match_length': 10}
    ]
    errors = data_processing.validate_match_data(match_data)
    assert errors == []
    # Missing required field
    bad_data = [{'player_name': 'Alice', 'kills': 5, 'deaths': 2, 'score': 100, 'weapon': 'AR'}]
